...
static void help()
{
  ...
  std::cerr << "sets, ways and blocksize powers of two and blocksize at least 8." << std::endl;
  exit(1);
}
...
// OVERLOAD for our PA3. DO NOT MODIFY
void cache_sim_t::access(uint64_t addr, size_t bytes, bool store, bool verbose) 
{
  store ? write_accesses++ : read_accesses++;
  (store ? bytes_written : bytes_read) += bytes;
  ...
  {
    if(verbose) {
      std::cout << (store?"S ":"L ") << std::hex << addr << ',' << std::dec << bytes << " hit " << std::endl;
    }
  ...
  if(verbose) {
    std::cout << (store?"S ":"L ") << std::hex << addr << ',' << std::dec <<bytes << " miss " << ((victim&VALID)?"eviction ":"") << std::endl;
  }
