...
class cache_sim_t
{
 public:
  ...
  void access(uint64_t addr, size_t bytes, bool store, bool verbose);
  void clean_invalidate(uint64_t addr, size_t bytes, bool clean, bool inval);
  void print_stats();
  void set_miss_handler(cache_sim_t* mh) { miss_handler = mh; }
  void set_log(bool _log) { log = _log; }

  // helpers for wrapper to get stats
  uint64_t get_hits() { return (read_accesses+write_accesses-read_misses-write_misses);}
  uint64_t get_misses() { return (read_misses+write_misses);}
  uint64_t get_evictions() { return evictions; }
